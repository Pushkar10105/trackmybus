...
export default function AdminPage() {
  ...
  const [editBusDriverPhone, setEditBusDriverPhone] = useState('');
  const [editBusStatus, setEditBusStatus] = useState('active');
  ...
  const handleCreateBus = async (e) => {
    ...
    try {
      ...
      setActionNotice({ type: 'success', text: `Bus ${newBusNumber.trim()} successfully added to fleet!` });
      refreshData();
      setTimeout(() => setActionNotice(null), 3000);
    } catch (err) {
  ...
  const handleOpenEditBus = (bus) => {
    setEditingBus(bus);
    setEditBusNumber(bus.bus_number || '');
    setEditBusRouteId(bus.route_id ? String(bus.route_id) : '');
    setEditBusDriverPhone(bus.driver_phone || '');
    setEditBusStatus(bus.status || 'active');
  ...
  const handleUpdateBus = async (e) => {
    ...
    try {
      await adminApi.updateBus(editingBus.id, {
        ...
        route_id: editBusRouteId ? Number(editBusRouteId) : null,
        driver_phone: editBusDriverPhone.trim() || null,
        status: editBusStatus,
      });
      setEditBusModalOpen(false);
      setEditingBus(null);
      setActionNotice({ type: 'success', text: `Bus ${editBusNumber.trim()} updated successfully!` });
      refreshData();
      setTimeout(() => setActionNotice(null), 3000);
    } catch (err) {
      alert(err.message || 'Error updating bus');
    }
  };

  // Delete Bus
  const handleDeleteBus = async (id, busNumber) => {
    if (!window.confirm(`Are you sure you want to remove bus ${busNumber || id} from the fleet?`)) return;
    try {
      await adminApi.deleteBus(id);
      setActionNotice({ type: 'success', text: `Bus ${busNumber || id} removed from fleet!` });
      refreshData();
      setTimeout(() => setActionNotice(null), 3000);
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-slate-100 py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100">
                      {buses.length} {buses.length === 1 ? 'Vehicle' : 'Vehicles'}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500">
                    Vehicle registry and designated driver operational assignments identified by phone number.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={refreshData}
                    className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 transition"
                  >
              ...
              {buses.length === 0 ? (
                <div className="text-center py-12 text-slate-400 text-sm">
                  ...
                  No buses registered in the database yet. Click "Register Bus" to add one.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs text-slate-600">
                    <thead className="bg-slate-50 text-slate-700 uppercase font-bold text-[11px] border-b border-slate-200">
                      <tr>
                        ...
                        <th className="py-3 px-4">Driver (Identified by Phone)</th>
                        ...
                        <tr key={b.id} className="hover:bg-slate-50/70 transition">
                          <td className="py-3.5 px-4">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                <span className="text-[10px] text-slate-400">Asset #{b.id}</span>
                              </div>
                            </div>
                          </td>
                          <td className="py-3.5 px-4">
                            ...
                            {b.driver_phone ? (
                              <div className="space-y-1">
                                <div className="flex items-center gap-1.5 font-bold text-slate-800">
                                  ...
                                  <span>{b.driver_name || 'Driver'}</span>
                                </div>
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-indigo-50 border border-indigo-200/80 text-indigo-700 font-mono font-bold text-[11px]">
                                  <Phone className="w-3 h-3 text-indigo-500" />
                                  <span>{b.driver_phone}</span>
                                </div>
                              </div>
                            ) : (
                              <div className="inline-flex items-center gap-1 px-2 py-1 rounded bg-amber-50 text-amber-700 border border-amber-200/60 text-[11px]">
                            ...
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${
                                  b.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'
                                }`}
                              />
                              {b.status || 'active'}
                            </span>
                          ...
                          <td className="py-3.5 px-4 text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <button
                                onClick={() => handleDeleteBus(b.id, b.bus_number)}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="e.g. 9000000002 or select from list"
            ...
            {newBusDriverPhone && (
              <div className="mt-1">
                {(() => {
                  const matched = drivers.find((d) => d.phone === newBusDriverPhone.trim());
                  if (matched) {
                    return (
                      <span className="text-[11px] text-emerald-600 font-semibold flex items-center gap-1">
                        <UserCheck className="w-3.5 h-3.5" />
                        Identified Driver: {matched.name}
                      </span>
                    );
                  }
                  return (
                    <span className="text-[11px] text-slate-400">
                      Will verify driver phone in database on submission.
                    </span>
                  );
                })()}
              </div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Identifies the driver by phone number in the system.
            </p>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button
      ...
      {/* MODAL: EDIT BUS */}
      <Modal isOpen={editBusModalOpen} onClose={() => setEditBusModalOpen(false)} title="Edit Fleet Bus">
        <form onSubmit={handleUpdateBus} className="space-y-3 text-xs">
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">
              Driver Phone Number (Optional - clear to unassign)
            </label>
            <div className="relative">
              <input
                type="tel"
                list="edit-drivers-list"
                placeholder="Enter driver phone or select from list"
                value={editBusDriverPhone}
                onChange={(e) => setEditBusDriverPhone(e.target.value)}
                className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-mono"
              />
            ...
            {editBusDriverPhone && (
              <div className="mt-1">
                {(() => {
                  const matched = drivers.find((d) => d.phone === editBusDriverPhone.trim());
                  if (matched) {
                    return (
                      <span className="text-[11px] text-emerald-600 font-semibold flex items-center gap-1">
                        <UserCheck className="w-3.5 h-3.5" />
                  ...
                  return (
                    <span className="text-[11px] text-slate-400">
                      Must match a driver registered with this phone number.
                    </span>
                  );
                })()}
              </div>
